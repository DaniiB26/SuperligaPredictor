...
const MyPredictions: React.FC = () => {
    ...
    const getVisibleStages = () => {
        if (currentIndex === 0) {
            ...
            return stages.slice(Math.max(0, currentIndex - 1), Math.min(currentIndex + 2, stagesLength));
        }
    };
        ...
        const fetchData = async () => {
            if (user) {
                try {
                    ...
                    const groupedMatches = matchData.reduce((acc: { [key: string]: Match[] }, match: Match) => {
                        if (!acc[match.etapa]) {
                            acc[match.etapa] = [];
                        }
                    ...
                    const totalUserPoints = predictionData.reduce((sum, prediction) => sum + prediction.points, 0);
                    ...
                    for (let index = 0; index < Object.keys(groupedMatches).length; index++) {
                        ...
                        const upcomingMatch = stageMatches.find(match => match.status !== 'completed');
                        if (upcomingMatch) {
    ...
    const findUserPrediction = (matchId: string) => {
        return predictions.find(prediction => prediction.match.id === matchId);
    };
    ...
    const handleRefresh = async () => {
        if (user) {
            try {
                ...
                const updatedTotalPoints = updatedPredictions.reduce((sum, prediction) => sum + prediction.points, 0);
        ...
        <div className="my-predictions">
            ...
            <div className="match-list-wrapper" {...swipeHandlers}>
                <div className="match-list">
                    {matches[selectedStage] && matches[selectedStage].map(match => (
